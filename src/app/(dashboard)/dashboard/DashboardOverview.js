    ...
    <div className="flex flex-col gap-6">
      ...
      <Card className="border-2 border-border shadow-[3px_3px_0px_var(--color-border)] p-4 sm:p-6 flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="inline-flex p-1 rounded border-2 border-border bg-surface-2 shadow-[2px_2px_0px_var(--color-border)]">
      ...
      {/* Visual Analytics: Top Models Bar Chart & Provider / Status Distribution */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Provider Distribution Pie Chart (1 Col) */}
        <Card className="border-2 border-border shadow-[3px_3px_0px_var(--color-border)] p-4 sm:p-6 flex flex-col gap-4">
          <div>
            <h2 className="text-sm font-bold uppercase tracking-wider text-text-main flex items-center gap-2">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Legend
                    ...
                    height={36}
                    iconType="circle"
                    formatter={(val) => <span className="text-[11px] font-bold text-text-main">{val}</span>}
                  />
                </PieChart>
              </ResponsiveContainer>
            )}
      ...
      <Card padding="none" className="border-2 border-border shadow-[3px_3px_0px_var(--color-border)] overflow-hidden">
        <div className="p-4 sm:p-6 border-b-2 border-border flex items-center justify-between">
          <div>
            <h2 className="text-sm font-bold uppercase tracking-wider text-text-main flex items-center gap-2">
          ...
          <Link href="/dashboard/usage?tab=details">
          ...
          <table className="w-full min-w-[720px]">
            <thead>
              <tr className="border-b-2 border-border bg-black/[0.02] dark:bg-white/[0.02]">
                ...
                <th className="text-right p-4 text-xs font-bold uppercase tracking-wider text-text-main">Tokens</th>
                <th className="text-right p-4 text-xs font-bold uppercase tracking-wider text-text-main">Est. Cost</th>
                <th className="text-center p-4 text-xs font-bold uppercase tracking-wider text-text-main">Status</th>
              </tr>
            </thead>
              ...
              {recentRequests.length === 0 ? (
                ...
                recentRequests.map((req) => (
                  <tr key={req.id} className="hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="p-4 text-text-main font-bold truncate max-w-[200px]">
                      {req.model}
                    </td>
                    <td className="p-4 text-text-muted truncate max-w-[150px]">
                      {req.provider}
                    </td>
                    <td className="p-4 text-right text-text-main font-bold">
