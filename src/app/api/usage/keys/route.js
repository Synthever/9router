 ...
 * Returns aggregated usage statistics grouped by each API Key, plus summary breakdown per model & endpoint.
 */
  ...
  try {
    ...
    for (const r of rows) {
      ...
      if (r.apiKey && keyMap[r.apiKey]) {
        ...
        // Deleted or external key that has history
        if (!keyMap[r.apiKey]) {
          keyMap[r.apiKey] = {
